    ...
    private async Task HandleClientAsync(TcpClient client)
    {
        ...
        using (var stream = client.GetStream())
        {
            ...
            try
            {
                ...
                else if (command == "assignpatient")
                {
                    var (assignSuccess, message) = await _roomService.AssignPatientToRoom(
                    ...
                    else if (command == "getroompatients")
                    {
                        var patients = await _roomService.GetPatientsInRoom(
                            int.Parse(requestData["roomId"]));
                    var response = new
                    {
                        success = true,
                        message = "Room patients retrieved successfully",
                        patients = patients
                    };
                    var jsonResponse = JsonSerializer.Serialize(response, new JsonSerializerOptions
                    {
                ...
                else if (command == "removepatient")
                {
                    var (success, message) = await _roomService.RemovePatientFromRoom(
                        int.Parse(requestData["patientId"]),
                        int.Parse(requestData["roomId"]));
